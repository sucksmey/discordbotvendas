...
class SalesCog(commands.Cog):
    ...
    class PrePurchaseConfirmationView(View):
        ...
        async def confirm_callback(self, button: discord.ui.Button, interaction: discord.Interaction):
            ...
            await self.original_interaction.edit_original_response(content="✅ Confirmado! Criando seu carrinho...", view=self, embed=None)
    ...
    async def create_robux_thread(self, interaction: discord.Interaction):
        ...
        def chk(m): return m.author.id == user.id and m.channel.id == thread.id
        try:
            nick_msg = await self.bot.wait_for('message', check=chk, timeout=172800.0)
            nickname = nick_msg.content
            await thread.send(f"Ok, **{nickname}**! Qual a **quantidade de Robux** você deseja?")
            amt_msg = await self.bot.wait_for('message', check=chk, timeout=172800.0)
            amount = parse_robux_amount(amt_msg.content)
            ...
            desc = f"**Nickname:** `{nickname}`\n**Quantidade:** `{amount}` Robux\n"
            desc += f"**Valor a pagar:** `R$ {price:.2f}`\n\n"
            ...
            if customer_role and isinstance(user, discord.Member):
                await user.add_roles(customer_role, reason="Enviou comprovante de pagamento")
