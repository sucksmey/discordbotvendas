...
async def setup(bot: commands.Bot):
    # Não vamos registrar os listeners aqui, eles serão registrados no common_listeners.py
    # await bot.add_cog(Purchase(bot)) # Este cog agora é comum_listeners
    await bot.add_cog(RobuxCog(bot))
    await bot.add_cog(JogosCog(bot))
    await bot.add_cog(GiftcardCog(bot))
    await bot.add_cog(CommonListenersCog(bot))
