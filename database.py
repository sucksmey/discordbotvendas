...
async def init_db():
    ...
    async with pool.acquire() as c:
        # As tabelas continuam as mesmas, a lógica de conversão é no código
        await c.execute("""
...
# --- FUNÇÕES ATUALIZADAS COM A CORREÇÃO DE VALOR ---

async def add_purchase(user_id: int, product_name: str, price: float, attendant_id: int, deliverer_id: int | None):
    """Adiciona uma compra ao DB, convertendo o valor para centavos para evitar erros."""
    async with pool.acquire() as c:
        # CONVERSÃO: Salva o valor multiplicado por 100
...
async def get_user_spend_and_count(user_id: int):
    ...
    async with pool.acquire() as c:
        ...
        # CONVERSÃO: Divide o valor por 100 para obter o valor em Reais
...
async def get_purchase_history(user_id: int):
    ...
    async with pool.acquire() as c:
        # CONVERSÃO: Divide o valor por 100 diretamente na query SQL
